...
- Named-user Enterprise Dashboard authentication, MFA, roles, and administrator audit history
- Customer-portal, Escrow-event, billing, monitoring, and support-ticket ingestion into the Admin Dashboard

These items must not be described as generally available until a dated release entry confirms the exact scope.

## 15 September 2026

### Public controlled-pilot and Admin Dashboard synchronization

**Pilot** · **Documentation**

- Confirmed the unified product gateway, business-customer sign-in, white-labelled property-payment demonstration, SafeReceive decision journey, and branded comprehensive reports on the public Cardinal site.
- Confirmed the password-gated Enterprise Admin Dashboard is reading live Protection API pilot data.
- Confirmed operational views for protection checks, risk-report metadata, API usage, partners, masked API-key records, billing readiness, SafeSend fees, Escrow readiness, and support readiness.
- Added a dedicated Enterprise Admin Dashboard guide with metric definitions, data boundaries, operational gaps, and production controls.
- Clarified that dashboard reliability and latency figures describe recorded pilot traffic and are not an SLA.
- Clarified that `local_heuristic` or fixture evidence is not licensed production intelligence about a real entity.
- Clarified that an API-key environment labelled `PRODUCTION` does not prove mainnet settlement or production customer readiness.

Customer-portal records, Escrow contract events, support tickets, approved billing plans, external monitoring, named administrator identities, MFA, roles, and an administrator audit trail remain unreleased.

## 13 September 2026

### Business access and evidence documentation

**Pilot** · **Documentation**

- Published documentation for the separate Cardinal product gateway and business-customer sign-in routes.
- Added the white-labelled customer-portal payment-request journey and organisation-separation requirements.
- Documented estate agency, exchange, custodian, treasury-vault, wallet, and payment-platform use cases without implying universal production availability.
- Expanded comprehensive-report guidance for source-of-funds summaries, exposure paths, exchanges, custodians, institutions, jurisdictions, references, and freshness.
- Added a strict rule that Cardinal must not infer a bank or bank account from blockchain activity alone.
- Clarified which application and reporting changes are controlled demonstrations or feature-branch implementations pending production publication.
- Documented the Admin Dashboard's connected Protection API and report coverage, plus the remaining customer-portal, Escrow-event, and support-ticket integrations.
- Confirmed `contact@cardinalweb3.com` as the published support route without claiming that a dedicated mailbox, ticket system, or SLA already exists.

## 12 September 2026

### SafeReceive controlled investor pilot

**Pilot** · **Documentation**

- Connected direction-aware incoming-payment screening to normalized Cardinal Intelligence evidence and the Enterprise Policy Engine.
- Published controlled `ALLOW`, `REVIEW`, and `BLOCK` investor scenarios.
- Added explicit evidence source, confidence, freshness, provider status, and matched-policy-rule visibility.
- Added a reserved BLOCK scenario with clearly labelled synthetic stolen-funds, sanctions-list, and illicit-service evidence.
- Added immutable Comprehensive Risk Reports linked to the original protection-check request.
- Added report download and print/PDF controls and separate report metering.
- Removed duplicate risk explanations discovered during live production verification.

The controlled fixture is not evidence about a real wallet, person, business, or bank account. Licensed production intelligence coverage, customer pricing, payment collection, and treasury settlement are not represented as generally available.
...
Launched and expanded Cardinal's public Mintlify documentation from the [`CardinalKennyS/PublicDocs`](https://github.com/CardinalKennyS/PublicDocs) repository.
